...
Minimum Phaseのハイパスフィルターとローパスフィルター、出力ゲイン、フィルター応答表示だけを提供する。一般的なデジタルEQと遜色ないButterworth応答、クリックレスな操作、ゼロレイテンシー、再現可能な状態保存を重視する。
...
利用者は、HP On、HP Pole、HP Freq、LP On、LP Pole、LP Freq、Output Gainの全項目をホストからオートメーションできる。
...
* 初期サイズは720×420 pxである
* 上段約60%を応答グラフ、下段をHP、LP、Outputの3区画とする
* HP/LPはOn/Off、Pole選択、Freqノブ、編集可能な数値欄を持つ
...
* 処理済みステレオ音声: HP、LP、Output Gainを順に適用した音声ブロック
* 保存状態: スキーマバージョンと全パラメータを含むシリアライズデータ
* 応答表示: UI側で計算したHP/LP合成応答

### Persistence

* APVTSへ7個のパラメータとOn/Off状態を保存する
...
* HP、LP、Outputの操作区画
* HP/LP合成応答グラフ
* APVTS Attachmentを介したノブ、数値欄、On/Off、Pole選択
...
* 7個のパラメータをUIとホストオートメーションから操作できる
...
* Pole、On/Off、Freq、Gain変更で実用上明確なクリックや発振が生じる
...
5. 低周波・8 poleを含むPole切替、Freq掃引、On/Off、Gain変更を聴感確認する
